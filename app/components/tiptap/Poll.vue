...
import type { BoPollViewListModel, BoPollViewModel } from "models";
...
const searchResults = ref<BoPollViewListModel[]>([]);
const isSearching = ref(false);
const selectedPoll = ref<BoPollViewListModel | null>(null);

// Preview data
const pollDetail = ref<BoPollViewModel | null>(null);
...
const loadPollDetail = async (reference: string) => {
  ...
  try {
    const results = await getPollsData({ reference });
    if (results.length > 0) {
      pollDetail.value = await getPoll(results[0].id);
    } else {
